...
police take over certain intersections to control the pedestrian flow such that
pedestrians and vehicles take turns using the intersections (i.e.,
overrule traffic lights). The third is placing signs along the sidewalk to
direct pedestrians to walk a certain direction to their destination (i.e.,
pedestrians do not use the intersection at all). The optimal time for everyone
...
that is closed to all vehicles may be a combination that produces better
...
intersections are shown in Fig. \ref{fig:intersections}.

\begin{figure}[H]
  \includegraphics[width=\linewidth,natwidth=1287,natheight=943]{GATechMap_IntersectionIDs.png}
  \caption{SUI Intersections.}
...
combination; this is shown in (XXXX REFERENCE TABLE HERE).  Green cells indicate
an intersection closed to vehicular traffic (i.e., it is effectively a sidewalk),
yellow cells indicate an intersection with alternating vehicular and pedestrian
traffic (i.e., a law enforcement officer directing traffic), and red cells indicate
an intersection closed to all pedestrian traffic (i.e., nodes are removed from
the map).

(ADD TABLE OF INTERSECTION CONFIGURATIONS HERE)
